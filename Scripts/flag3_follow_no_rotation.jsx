// Pattern follow-without-rotation dla pary: -3_flag_UK 2 -> 3_PLANE_df_przechwyc_bitwa.
// Offset (dx, dy) kopiujemy z istniejącego expressiona na 2_flag_UK,
// żeby flaga miała taki sam względny offset jak w poprzedniej parze.
(function () {
    var FLAG_NAME    = "-3_flag_UK 2";
    var PLANE_NAME   = "3_PLANE_df_przechwyc_bitwa";
    var SRC_FLAG     = "2_flag_UK";   // źródło offsetu
    ...
    if (!src)   { alert("Brak " + SRC_FLAG + " — skąd skopiować offset?"); return; }

    // Wyciągnij dx, dy z expressiona src.Position
    var srcPos = src.property("ADBE Transform Group").property("ADBE Position");
    var srcExpr = srcPos.expression || "";
    var m = srcExpr.match(/\+\s*\[\s*(-?[\d.]+)\s*,\s*(-?[\d.]+)\s*\]/);
    if (!m) { alert("Nie udało się odczytać dx,dy z expressiona " + SRC_FLAG); return; }
